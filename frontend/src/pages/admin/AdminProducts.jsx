    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
        ...
        <Link 
          ...
          className="flex items-center space-x-2 bg-primary hover:bg-blue-700 text-white px-8 py-4 rounded-2xl transition-all shadow-xl shadow-primary/20 font-bold"
        >
          <Plus size={20} />
        ...
        <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/30">
          <div className="relative w-full md:w-[450px]">
            ...
            <input
              ...
              className="w-full pl-12 pr-6 py-4 bg-white border border-slate-200 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none font-medium shadow-sm"
              value={searchTerm}
        ...
        {loading ? (
          <div className="p-32 flex flex-col items-center justify-center space-y-6">
            <div className="relative">
              ...
              <Package className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-primary/40" size={24} />
            </div>
            <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-xs">Synchronizing Database...</p>
          </div>
        ) : error ? (
          <div className="p-32 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <button onClick={fetchProducts} className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-slate-800 transition-colors">Retry Connection</button>
          </div>
          ...
          <div className="p-32 text-center flex flex-col items-center">
            ...
            <p className="text-slate-500 mt-2">No products match your current search parameters.</p>
          </div>
        ) : (
                ...
                {filteredProducts.map((product) => (
                  <tr key={product._id} className="hover:bg-primary/[0.02] transition-colors group">
                    ...
                    <td className="px-8 py-6 text-center">
                      <div className="flex flex-col items-center space-y-1.5">
                        ...
                        <div className="flex space-x-2">
                           {product.isFeatured && <span className="w-2 h-2 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]" title="Featured"></span>}
                           {product.isNewArrival && <span className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]" title="New Arrival"></span>}
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6 text-right">
                      ...
                      <button className="p-3 text-slate-400 md:hidden">
